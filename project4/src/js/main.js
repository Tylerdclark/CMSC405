...
import { project, translate, rotateX as _rotateX, rotateY as _rotateY, rotateZ as _rotateZ, scale, multiply } from './matrix.js';
...
const vShader = creator.shader(gl, gl.VERTEX_SHADER, vShaderSource);
const fShader = creator.shader(gl, gl.FRAGMENT_SHADER, fShaderSource);
const program = creator.program(gl, vShader, fShader);
...
function draw() {
  ...
  var projectionMat = project(gl.canvas.width, gl.canvas.height, 1000);
  var translationMat = translate(params.translateX, params.translateY, params.translateZ);
  var rotateXMat = _rotateX(params.rotateX);
  var rotateYMat = _rotateY(params.rotateY);
  var rotateZMat = _rotateZ(params.rotateZ);
  var scaleMat = scale(params.scaleX, params.scaleY, params.scaleZ);

  let transformation = multiply(scaleMat, rotateXMat);
  transformation = multiply(transformation, rotateYMat);
  transformation = multiply(transformation, rotateZMat);
  transformation = multiply(transformation, translationMat);
  transformation = multiply(transformation, projectionMat);
  //set transformation

  gl.uniform1f(fudgeLocation, params.fudgeFactor);
  gl.uniformMatrix4fv(matrixUniformLocation, false, transformation);
  // Set the color to use
  gl.uniform4fv(colorLocation, [1, 0.1, 0.1, 1]);
